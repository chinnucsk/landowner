...
start_link(ParentPid) ->
	gen_server:start(?MODULE, ParentPid, []).
...
handle_call(match, {CallId,_} = From, State) ->
	{reply,ok,State}.

handle_info({'EXIT', Pid, _Reason}, #state{parent_pid=Pid} = State) ->
